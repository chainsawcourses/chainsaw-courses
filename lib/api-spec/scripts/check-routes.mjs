...
function extractServerRoutes(dir) {
  /** @type {Map<string, string[]>} key = "METHOD /path", value = array of source filenames */
...
function extractSpecRoutes(yamlPath) {
...
function main() {
  ...
  // ---------------------------------------------------------------------------
  // Step 4: Check for stale exemptions (entries in EXEMPT_ROUTES that no
  //         longer match any actual server route).
  // ---------------------------------------------------------------------------
  const staleExemptions = [];

  for (const exemption of EXEMPT_ROUTES) {
