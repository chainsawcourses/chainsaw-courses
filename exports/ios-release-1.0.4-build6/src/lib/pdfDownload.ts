...
const pdfSaverRegistry = globalThis as typeof globalThis & {
  __chainsawCoursesPdfSaver?: PdfSaverPlugin;
};
const PdfSaver = pdfSaverRegistry.__chainsawCoursesPdfSaver
  ?? registerPlugin<PdfSaverPlugin>("PdfSaver");
pdfSaverRegistry.__chainsawCoursesPdfSaver = PdfSaver;
...
async function deliverNativeFile(
  ...
  // Android 10+ and iOS can save directly into user-visible file storage.
  // This avoids relying on a share chooser, which can be missing or blocked
  // on some devices and WebViews.
  if (Capacitor.getPlatform() === "android" || Capacitor.getPlatform() === "ios") {
    try {
      await PdfSaver.save({ data, filename: safeName });
      return "saved";
    } catch (error) {
      console.warn("Direct native PDF save failed; trying Share fallback", error);
    }
  }
